...
- Both support direct go-agents usage (primary) and hub coordination (optional)
...
**Purpose**: Show primary usage pattern with go-agents (no hub required)
...
**Code Structure**:
```go
// Create go-agents agent (from config)
...
**Integration Value**:
- Validates go-agents integration (primary pattern)
...
8. **Direct Agent Integration** - 60 min
   - Primary pattern with go-agents
   - Multi-modal example
